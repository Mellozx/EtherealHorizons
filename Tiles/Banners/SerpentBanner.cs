...
namespace EtherealHorizons.Tiles.Banners     //We need this to basically indicate the folder where it is to be read from, so you the texture will load correctly
{
    public class SerpentBanner : ModTile
    {
        public override void SetDefaults()
        {
			...
			name.SetDefault("Serpent Banner");
            AddMapEntry(new Color(201, 163, 80), name); //this defines the color and the name when you see this tile on the map
        }
 
        public override void KillMultiTile(int i, int j, int frameX, int frameY)
        {
            Item.NewItem(i * 16, j * 16, 16, 48, mod.ItemType("SerpentBanner"));//this defines what to drop when this tile is destroyed
        }
        ...
        public override void NearbyEffects(int i, int j, bool closer)   //this make so the banner give an effect to nearby players
        {
            if (closer)          //so if a player is close to the banner
            {
                ...
                player.NPCBannerBuff[mod.NPCType("Serpent")] = true;	
